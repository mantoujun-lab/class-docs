...
- [门店信息表.XLSX](/data/门店信息表.XLSX)
- [全国销售订单数量表.xlsx](/data/全国销售订单数量表.xlsx)

> 注:本节还用到了 `会员信息查询.xlsx` 与 `会员消费报表.xlsx`,由于这两份数据包含真实会员的卡号、生日、消费明细等敏感个人信息,涉及《个人信息保护法》定义的敏感个人信息,本站不提供下载。请使用脱敏或合成数据替代进行练习。
:::
